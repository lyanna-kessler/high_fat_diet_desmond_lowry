...
# biplots: can make one for each group (treat, diet, time)
# maybe one that's affected by both diet_treat
...
# add the taxonomy name
res$name <- tax.map[res$taxon]
res_nicknames <- give_nickname(names = res$name, genus = TRUE)
res$nickname <- res_nicknames
...
#### Genus: Plotting
```{r}
top_g <- read_csv("data/biplot_table_Hfd_t1-4_lfc3_raw.csv")
top_g$taxon <- as.factor(top_g$taxon)
...
# Timepoint 1vs2: Timepoint1 and Diettime1
m_12 <- filter(meta,
               Timepoint == "1" | Timepoint == "2")
rownames(m_12) <- m_12$SampleID
...
t12_rare <- filter(rare_df,
                   SampleID %in% m_12$SampleID)

rare_12 <- transpose(t12_rare)
rownames(rare_12) <- colnames(t12_rare)
colnames(rare_12) <- rownames(t12_rare)

asvs_12 <- rare_12[(row.names(rare_12) %in% top_asvs$Feature.ID),]
asvs_12$ASV <- rownames(asvs_12)


asvs <- rownames(asvs_12)
...
asvs_12$Genus <- genera
asvs_12$Genus <- as.factor(asvs_12$Genus)

top_g_12 <- filter(top_g,
                   significance_filter == "q_DietTime1" | significance_filter == "q_Timepoint1")

top_asvs_12 <- filter(asvs_12,
                      Genus %in% top_g_12$taxon)

# reset levels of Genus by changing it to character then factor again
top_asvs_12$Genus <- as.character(top_asvs_12$Genus)
top_asvs_12$Genus <- as.factor(top_asvs_12$Genus)
top_asvs_12$UniqueG <- NA

for (i in 1:nrow(top_asvs_12)){
  name <- as.character(top_asvs_12$Genus[i])
  top_asvs_12$UniqueG[i] <- paste(name, "__", as.character(i), sep="")
}


rownames(top_asvs_12) <- top_asvs_12$UniqueG
filter_12 <- select(top_asvs_12, -Genus, -ASV, -UniqueG)
top_12 <- transpose(filter_12)
rownames(top_12) <- colnames(filter_12)
colnames(top_12) <- rownames(filter_12)

# Remove columns that sum in 0
top_12 <- mutate_all(top_12, function(x) as.numeric(as.character(x)))
top_12 <- top_12[colSums(abs(top_12), na.rm = TRUE) > 0]



m_12_bac <- merge(top_12, m_12, 
                  by = 'row.names', all = TRUE)

rownames(m_12_bac) <- m_12_bac$Row.names
m_12_bac <- select(m_12_bac, -Row.names)

# specific to this dataframe, where bacteria are 1:21
mbac_long <- pivot_longer(m_12_bac,
                          cols = 1:21,
                          names_to = c(".value","Number"),
                          names_sep = "__")
mbac_long <- select(mbac_long, -Number)
mbac_long$TreatDiet <- paste(mbac_long$Treatment, "+", mbac_long$Diet)

ggplot(mbac_long, aes(x=Timepoint, y=`Clostridia_UCG-014`, fill=TreatDiet)) +
...
ggplot(mb_ll, aes(x=Timepoint, y=Relative_Abundance, fill=TreatDiet)) +
  geom_bar(color = "black",
           stat="summary",
           fun = mean,
           position=position_dodge(.95))+
...
#ggsave("figures/top_genera_ANCOM_panels.png", width = 16, height = 8, units = "in")
```
...
top_rare <- rare[rare$Phylum %in% top_p$taxon,]
...
#ggsave("figures/top_phylum.png", width = 12, height = 8, units = "in")
```
...
#ggsave("figures/modules_diettime1-4.png", width = 10, height = 8, units = "in")
# 
